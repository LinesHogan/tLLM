...
_TUTORIAL = _REPO_ROOT / "doc" / "guides" / "consumer-extension.md"
_VALIDATION = _REPO_ROOT / "doc" / "guides" / "validation.md"


class DocsLinksUnitTest(unittest.TestCase):
    def test_readme_links_consumer_tutorial(self) -> None:
        text = _README.read_text(encoding="utf-8")
        self.assertIn("guides/consumer-extension.md", text)
        self.assertIn("guides/validation.md", text)
    ...
    def test_dummy_tutorial_mentions_read_write_demo(self) -> None:
        text = _TUTORIAL.read_text(encoding="utf-8")
        self.assertIn("read/write hidden", text)
